(() => {
    ...
    try {
        name_origin_film = $('.b-post__origtitle')[0].textContent.replaceAll(" ", "_").replaceAll("_/_", "|")
    } catch (error) {
    ...
    let season_id = $('.b-simple_seasons__list .b-simple_season__item.active').data('tab_id');
    if (season_id !== null) {
        dictionary["season_id"] = season_id.toString();
    } else {
        dictionary["season_id"] = match[4];
    }
    let episode_id = $('.b-simple_episodes__list .b-simple_episode__item.active').data('episode_id');
    if (episode_id !== null) {
   ...
   dictionary["quality"] = CDNPlayer.api('quality');
   dictionary["qualities"] = CDNPlayer.api('qualities');
